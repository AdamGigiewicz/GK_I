...
public class Lab6 extends GLJPanel implements GLEventListener  {
	...
	public void display(GLAutoDrawable drawable) {    
		...
		gl2.glRotated( rotateY, 0, 1, 0 );   						 	// obrót piramidy względem osi Y 
	...
	public void Pyramid(GL2 gl2,int numVertices){					// tworzenie piramidy w podstawie sześciokąt
        	int n = 6;									
	        int r = 5;												// promień podstawy 												
	        float y = -1;											// pozycja początkowoa podstawy
	        int h = 7;												// wysokość piramidy 
	        float x,z;

	        gl2.glBegin(GL2.GL_TRIANGLES);

	        gl2.glMaterialfv( GL2.GL_FRONT_AND_BACK, GL2.GL_DIFFUSE, materials[1], 2);		 // [1] określa materiał 
	   		gl2.glMaterialfv( GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT, materials[1], 8);		// drugi parametr określa odcień/właściwości jako indeks w tablicy 
	        gl2.glMaterialfv( GL2.GL_FRONT_AND_BACK, GL2.GL_SPECULAR, materials[1], 7 );
	        ...
	        for (int i = 0; i <= n; i ++)
	        {
	            ...
	            gl2.glNormal3f(U[1]*V[2]-U[2]*V[1],U[2]*V[0]-U[0]*V[2],U[0]*V[1]-U[1]*V[0]); // wektor normlany dla obu wierchołków tego kąta 
	            gl2.glVertex3f(x0, y, z0);		
	            x0=x;
	            z0=z;
	            gl2.glVertex3f(x, y, z);
	            gl2.glVertex3f(0, h, 0);  //Vertex on the bottom edge.
	        }
	        gl2.glEnd();
	...
	public void init(GLAutoDrawable drawable) {
		...
		gl2.glEnable(GL2.GL_LIGHT0); 		// określa, które światło jest konfigurowane
		// TODO configure better lighting!
		float[] blue1  = {0.4f, 0.4f, 0.4f, 1};
		float[] blue2  = {0, 0, 0.8f, 1};
		float[] blue3  = {0.3f, 0.15f, 0.15f, 1};
		gl2.glLightfv (gl2.GL_LIGHT1, gl2.GL_DIFFUSE, blue1,0);		//gl.diffuse, ambient i specular informuje, która właściwośc światła jest ustawiona
		...
		gl2.glLightfv (gl2.GL_LIGHT1, GLLightingFunc.GL_POSITION, position, 0); //gl _position służy do wyboru rodzaju światła oraz ustawinia jego pozycji
