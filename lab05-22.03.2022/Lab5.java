...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
	...
	private void draw(GL2 gl2) {
		...
		gl2.glRotatef(rotateY,0,1,0);   // Apply rotations to complete object.		// dla  mojego wariantu obrót wokó³ osi Y 
																					// mo¿na obracaæ wokó³ tej osi korkoci¹g i piramide
		
		// TODO: Draw the currently selected object, number 1, 2, 3, 4, 5, or 6.
		if(objectNumber==1)															// switch do prze³¹czania figur 
		{
			gl2.glRotatef(-90, 1, 0, 0);
			//gl2.glColor3f(1, 0, 0);	// ledwo widaæ taki czerwony
			
			gl2.glColor3f(4,0,0);		// zmieni³em na ¿ywszy czerwony
	...
	private void korkociag(GL2 gl2) {						//tworzenie korkoci¹gu 
		
		for (int i =0; i<90; i++)							//okreœla iloœæ zwojów 
		{
			gl2.glPointSize(1+i/10);
			gl2.glBegin(GL_POINTS);
			gl2.glVertex3f(	(float)(2 * Math.cos(i * 2 * Math.PI / 30)),
							(float)(2 * Math.sin(i * 2 * Math.PI / 30)),0.08f * i);	//parametr z f odleg³oœæ pomiêdzy spiralami 
			gl2.glEnd();
		}
	}
	...
	{
		gl2.glBegin(GL_TRIANGLE_FAN);							// glBegin informuje, który prymityw jest rysowany , 
																//GL_TRIANGLE_FAN - zbiór wspólnych trójk¹tych maj¹cy wspólny wiercho³ek podawany dla funkcji

		gl2.glVertex3d(	(float)(Math.cos(j * 2 * Math.PI / n)),			// podaje kordynaty do wspólrzêdnych wierzcho³ka w siatce trójwymiarowej 
						(float)(Math.sin(j * 2 * Math.PI / n)), 2f);
		gl2.glVertex3d(	(float)(Math.cos(k * 2 * Math.PI / n)),
						(float)(Math.sin(k * 2 * Math.PI / n)), 2f);
		gl2.glVertex3d(0, 0, 0);

		gl2.glEnd();			//koñczy rysowanie prymitywu
	}

	private void piramida(GL2 gl2)				// tworzenie piramidy o podstawie 6 dla mojego wariantu szeœciok¹t
	{
		float size = 5;
		int n = 6;								// szeœciok¹t 
		gl2.glScalef(size, size, size);
		gl2.glRotatef(90, 1, 0, 0);				// obrót o 90 stopni wzglêdem osi X i wtedy jestt pionowo 
		gl2.glTranslatef(0, 0, -1);				// dziêki temu tworzy siê na œrodku pola
